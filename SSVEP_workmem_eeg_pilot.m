...
%     pl.data(:,:,i_contr,:)=ERP_data(:,pl.tidx(1):pl.tidx(2),pl.contrast{i_contr}(1),:);
%     pl.data(:,:,i_contr,:)=ERP_data(:,pl.tidx(1):pl.tidx(2),pl.contrast{i_contr}(2),:);
end
...
%% do some plotting CDA EEG contrast differences (cue left - cue right) electrode cluster (left - right)
pl.time2plot = [-100 1000];
pl.tidx = dsearchn(ERP_times', pl.time2plot');
...
for i_contr = 1:size(pl.contrast,1)
    pl.data(:,i_contr,:)=(mean(ERP_data(pl.elec2plot_i{1},:,pl.contrast{i_contr}(1),:),1) - mean(ERP_data(pl.elec2plot_i{1},:,pl.contrast{i_contr}(2),:),1)) - ...
        (mean(ERP_data(pl.elec2plot_i{2},:,pl.contrast{i_contr}(1),:),1) - mean(ERP_data(pl.elec2plot_i{2},:,pl.contrast{i_contr}(2),:),1));
end
...
xlabel('time in ms')
ylabel('amplitude in \muV/cm²')
title("filtered ERP | difference between cue left and right for left - right cluster")
legend(p.con3label,"Interpreter","none","Location","Eastoutside")
...
% pl.elec2plot = {{'CP3';'CP5';'P3';'P5';'P7';'P9';'PO3';'PO7'}; {'CP4';'CP6';'P4';'P6';'P8';'P10';'PO4';'PO8'}};
pl.elec2plot = {{'PO4';'PO2';'PO8';'I2'}; {'PO3';'PO1';'PO7';'I1'}};
